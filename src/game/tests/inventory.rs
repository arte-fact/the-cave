...
fn iron_sword() -> Item {
    Item { kind: ItemKind::Weapon, name: "Iron Sword", glyph: '/', effect: ItemEffect::BuffAttack(5), weight: 2, durability: 35 }
}
...
fn chain_mail() -> Item {
    Item { kind: ItemKind::Armor, name: "Chain Mail", glyph: '[', effect: ItemEffect::BuffDefense(4), weight: 0, durability: 40 }
}
    ...
    fn defense_minimum_damage_is_one() {
        ...
        g.equipped_armor = Some(Item {
            ...
            effect: ItemEffect::BuffDefense(6), weight: 0, durability: 60,
        });
        let hp_before = g.player_hp;
        let gx = g.player_x + 1;
        let gy = g.player_y;
